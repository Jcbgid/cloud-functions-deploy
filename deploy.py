...
from dev_deploy_def import dev_deploy_vars, dev_deploy_spec, DEV_GCLOUD_UTIL_CONFIG
from prod_deploy_def import prod_deploy_spec, prod_deploy_vars, PROD_GCLOUD_UTIL_CONFIG

DEPLOY_DIRECTORY = "_temp_deploy"
LIB_DIRECTORY = ""
...
def main():
    ...
    if dep_env == "dev":
        ...
        env_vars = prod_deploy_vars
        deploy_spec = prod_deploy_spec
        gcloud_util_config = PROD_GCLOUD_UTIL_CONFIG
    else:
        raise Exception
    ...
    allow_unauthenticated_clause = "--allow-unauthenticated" if deploy_spec[cloud_function_name]['allow_unauthenticated'] else ""

    env_vars_clause = "--set-env-vars "
    for k, v in env_vars.items():
        env_vars_clause += "{}={},".format(k, v)

    subprocess.call("gcloud config configurations activate {}".format(gcloud_util_config), shell=True, stdout=subprocess.PIPE)
    ...
    proc = subprocess.call("gcloud functions deploy {} --entry-point {} --runtime python39 --trigger-http {} {}"
                            "".format(cloud_function_name, cloud_function_name, env_vars_clause, allow_unauthenticated_clause), shell=True,
